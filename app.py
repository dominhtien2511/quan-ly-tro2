...
# ==========================================
st.set_page_config(page_title="Hệ Sinh Thái Quản Lý Trọ V18", layout="wide", initial_sidebar_state="expanded")
...
def get_db_connection():
    # Sử dụng database v18 để làm mới cấu trúc
    conn = sqlite3.connect('dulieu_tro_v18.db', check_same_thread=False)
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS phong_tro (
        ...
        gia INTEGER DEFAULT 2500000, tien_coc INTEGER DEFAULT 0, dien_cu INTEGER DEFAULT 0, nuoc_cu INTEGER DEFAULT 0, 
        dien_moi INTEGER DEFAULT 0, nuoc_moi INTEGER DEFAULT 0, trang_thai TEXT DEFAULT 'Trống', thanh_toan TEXT DEFAULT 'Chưa'
    )''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS chi_phi (
        id INTEGER PRIMARY KEY AUTOINCREMENT, thang_nam TEXT, loai TEXT, hang_muc TEXT, so_tien INTEGER DEFAULT 0, ngay_nhap TEXT
    )''')
    c.execute('''CREATE TABLE IF NOT EXISTS bao_tri (
        id INTEGER PRIMARY KEY AUTOINCREMENT, phong TEXT, van_de TEXT, ngay_bao TEXT, trang_thai TEXT DEFAULT '🔴 Chưa sửa', chi_phi INTEGER DEFAULT 0
    )''')
    conn.commit()
    return conn, c
...
# Ngân hàng (Dành cho QR)
if IS_ADMIN:
    st.sidebar.markdown("---")
    st.sidebar.markdown("**🏦 Nhận tiền (QR)**")
...
if menu == "1. Quản lý Phòng & Tính tiền":
    ...
    if not df.empty:
        ...
        # SỬA LỖI TYPEERROR: Chuyển đổi về số an toàn
        ...
        if IS_ADMIN:
            st.markdown("---")
            sel_p = st.selectbox("👉 Chọn phòng thao tác:", ["-- Chọn --"] + df['phong'].tolist())
            if sel_p != "-- Chọn --":
                r = df[df['phong'] == sel_p].iloc[0]
                cl, cr = st.columns(2)
                with cl:
                    with st.form("edit_room"):
                        st.write("✏️ **Cập nhật khách**")
                        ten = st.text_input("Họ tên khách", value=r['khach_thue'])
                        gia = st.number_input("Giá thuê", value=int(pd.to_numeric(r['gia'], errors='coerce') or 2500000))
                        st.info("Khi nhấn Lưu, hệ thống sẽ tự động cập nhật tên này vào danh sách Thành viên.")
                        if st.form_submit_button("Lưu thông tin"):
                            c.execute('UPDATE phong_tro SET khach_thue=?, gia=?, trang_thai=? WHERE id=?', (ten, gia, "Đã thuê" if ten else "Trống", int(r['id'])))
                            conn.commit(); st.rerun()
                with cr:
                    with st.form("bill"):
                        st.write("🧾 **Chốt số & Tính tiền**")
                        ...
                        if st.form_submit_button("Tính tiền & Hiện QR"):
                            ...
                            st.code(f"Hóa đơn P.{sel_p}: {tong:,} VNĐ")
...
# =====================================================================
    ...
    if df_p.empty: st.info("Tháng này chưa có phòng nào được thuê.")
        ...
        with col1:
            ...
            # --- LOGIC TỰ ĐỘNG ĐỒNG BỘ CHỦ HỘ (QUAN TRỌNG) ---
            # Kiểm tra xem phòng này đã có "Chủ Hợp Đồng" trong bảng thanh_vien chưa
            check_chu = pd.read_sql_query(f"SELECT id, ten FROM thanh_vien WHERE phong='{sel_p_tv}' AND vai_tro='👑 Chủ Hợp Đồng'", conn)
            
            if ten_khach_chuan: # Nếu bên Menu 1 có tên khách
                if check_chu.empty:
                    # Nếu chưa có chủ hộ -> Thêm mới luôn
                    c.execute('INSERT INTO thanh_vien (phong, ten, vai_tro, cccd, sdt, que_quan) VALUES (?,?,?,?,?,?)', (sel_p_tv, ten_khach_chuan, '👑 Chủ Hợp Đồng', '', '', ''))
                    conn.commit()
                else:
                    # Nếu đã có nhưng tên khác (vừa đổi bên Menu 1) -> Cập nhật tên lại cho khớp
                    if check_chu.iloc[0]['ten'] != ten_khach_chuan:
                        c.execute('UPDATE thanh_vien SET ten=? WHERE id=?', (ten_khach_chuan, int(check_chu.iloc[0]['id'])))
                        conn.commit()
            
            if IS_ADMIN:
                with st.form("add_tv"):
                    st.write("➕ **Thêm người ở ghép**")
                    t_og = st.text_input("Tên người ở ghép")
                    if st.form_submit_button("Lưu"):
                        if t_og:
                            c.execute('INSERT INTO thanh_vien (phong, ten, vai_tro, cccd, sdt, que_quan) VALUES (?,?,?,?,?,?)', (sel_p_tv, t_og, '👤 Người ở ghép', '', '', ''))
                            conn.commit(); st.rerun()

        with col2:
            st.write(f"**Danh sách người ở Phòng {sel_p_tv}**")
                ...
                if IS_ADMIN:
                    ...
                    if st.button("Lưu thay đổi bảng"):
                        for _, row in res.iterrows():
                            if row['Xóa'] and row['vai_tro'] != '👑 Chủ Hợp Đồng': 
                                c.execute(f"DELETE FROM thanh_vien WHERE id={row['id']}")
                            else: 
                                c.execute('UPDATE thanh_vien SET cccd=?, sdt=?, que_quan=? WHERE id=?', (row['cccd'], row['sdt'], row['que_quan'], row['id']))
                        conn.commit(); st.rerun()
                else:
...
# =====================================================================
    ...
    with c1:
        if IS_ADMIN:
            with st.form("bt"):
                p_bt = st.selectbox("Phòng:", DANH_SACH_MAC_DINH)
                v_bt = st.text_area("Vấn đề")
                if st.form_submit_button("Ghi nhận"):
                    c.execute('INSERT INTO bao_tri (phong, van_de, ngay_bao) VALUES (?,?,?)', (p_bt, v_bt, datetime.now().strftime("%d/%m/%Y")))
                    conn.commit(); st.rerun()
    with c2:
        df_b = load_bao_tri()
...
# =====================================================================
    ...
    m_tk = get_header("📊 Báo Cáo Tháng")
    df_p = load_phong(m_tk)
    thu = 0
    if not df_p.empty:
        # Ép kiểu số an toàn để tính tổng
        ...
        df_ok = df_p[df_p['thanh_toan'] == 'Đã thanh toán']
        thu = sum([(r['gia'] + max(0, r['dien_moi']-r['dien_cu'])*GIA_DIEN + max(0, r['nuoc_moi']-r['nuoc_cu'])*GIA_NUOC + 100000) for _, r in df_ok.iterrows()])
    
    df_c = load_chi_phi(m_tk)
    chi = df_c['so_tien'].sum() if not df_c.empty else 0
    st.metric("LỢI NHUẬN", f"{thu-chi:,} VNĐ")
    st.dataframe(df_c, use_container_width=True)

elif menu == "5. Báo Cáo Tài Chính Năm":
    n_tk = st.selectbox("Năm", d_nam, index=d_nam.index(str(current_year)))
    data_y = []
    for i in range(1, 13):
        ts = f"{str(i).zfill(2)}/{n_tk}"
        df_py = load_phong(ts)
        t_y = 0
        if not df_py.empty:
            df_py['gia'] = pd.to_numeric(df_py['gia'], errors='coerce').fillna(0)
            t_y = df_py[df_py['thanh_toan'] == 'Đã thanh toán']['gia'].sum()
        data_y.append({"Tháng": i, "Thu": t_y})
    st.dataframe(pd.DataFrame(data_y), use_container_width=True, hide_index=True)
